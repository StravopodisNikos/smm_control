/*
 *  [26-12-24]   Node cpp file for [ridosc_simple] implementation of ridosc controller
 *               Robust Inverse Dynamics Operational Space Control: Robust controller for SMM 
...
void jointStatesCallback(const sensor_msgs::JointState::ConstPtr& joint_state, 
    ...
    if (joint_state->position.size() < DOF) {
        ROS_WARN("[updateJointState_simple/jointStatesCallback] Expected 3 joint positions, received %zu", joint_state->position.size());
        return;
    }

    // Check if joint state contains at least 3 velocities
    if (joint_state->velocity.size() < DOF) {
        ROS_WARN("[updateJointState_simple/jointStatesCallback] Expected 3 joint velocities, received %zu", joint_state->velocity.size());
        return;
    }
    ...
    //ROS_INFO("[updateJointState_simple/jointStatesCallback] Updating joint states with positions and velocities");
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "updateJointState_simple");
    ros::NodeHandle nh;

    // Debug statement
    //ROS_INFO("[updateJointState_simple] Initializing robot structure");

    // Initialize the robot structure
    RobotAbstractBase* robot_ptr = new Structure3Pseudos();

    // Debug statement
    //ROS_INFO("[updateJointState_simple] Initializing shared library");
    ...
    if (!my_shared_lib.initializeSharedLib()) {
        ROS_ERROR("[updateJointState_simple] Failed to initialize shared library.");
        return -1;
    }

    // Initialize the shared library for robot analytical solvers using screws
    //ROS_INFO("[updateJointState_simple] Initialized Shared Library.");
