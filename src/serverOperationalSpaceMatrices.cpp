...
void JointStateCallback(const sensor_msgs::JointState::ConstPtr& joint_state,  ScrewsKinematics& smm_robot_kin_solver, ScrewsDynamics& smm_robot_dyn_solver, ros::Publisher& jacobian_cond_pub) {
    ...
    try {
        ...
        ROS_ERROR("[serverOperationalSpaceMatrices/JointStateCallback] Failed to compute Operational Jacobian: %s", e.what());
    }
    ...
    try {
        if (jacob_cond_number < JACOB_COND_THRES) {
            // Simple inverse
            //ROS_INFO("[serverOperationalSpaceMatrices/JointStateCallback] Condition number is below threshold. Using simple inverse.");
            inv_jacobian = smm_robot_kin_solver.inverseOperationalSpaceJacobian_ptr();
        } else {
            // DLS inverse
            ROS_WARN("[serverOperationalSpaceMatrices/JointStateCallback] Condition number exceeds threshold. Using Damped Least Squares (DLS) inverse.");
        ...
        if (inv_jacobian) {
            ...
            ROS_ERROR("[serverOperationalSpaceMatrices/JointStateCallback] Failed to compute Jacobian inverse. Pointer is null.");
        }
    } catch (const std::runtime_error& e) {
        ROS_ERROR("[serverOperationalSpaceMatrices/JointStateCallback] Exception while computing Jacobian inverse: %s", e.what());
    }
    ...
    try {
        ...
        ROS_ERROR("[serverOperationalSpaceMatrices/JointStateCallback] Failed to compute Derivative Operational Jacobian: %s", e.what());
    }
...
bool sendDynamics(smm_control::GetOperationalSpaceMatrices::Request &req, smm_control::GetOperationalSpaceMatrices::Response &res) {
    ...
    if (req.get_Lambda) {
        ...
        //ROS_INFO("[serverOperationalSpaceMatrices/sendDynamics] Mass Matrix sent.");
    }
    ...
    if (req.get_op_jacobian) {
        ...
        //ROS_INFO("[serverOperationalSpaceMatrices/sendDynamics] Operational Jacobian matrix sent.");
    }

    // If client requests Coriolis Vectoer
    if (req.get_Gamma_OSD_Vector) {
        for (int i = 0; i < 3; i++) {
            res.Gamma_OSD_Vector[i] = Gamma_Vector(i);
        }
        //ROS_INFO("[serverOperationalSpaceMatrices/sendDynamics] Coriolis vector sent.");
    }
    ...
    if (req.get_Fg) {
        ...
        //ROS_INFO("[serverOperationalSpaceMatrices/sendDynamics] Gravity vector sent.");
    }
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "serverOperationalSpaceMatrices");
    ros::NodeHandle nh;
    ...
    ros::ServiceServer service = nh.advertiseService("GetOperationalSpaceMatrices", sendDynamics);
