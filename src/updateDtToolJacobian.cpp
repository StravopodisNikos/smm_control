...
void JointStateCallback(const sensor_msgs::JointState::ConstPtr& joint_state, 
    ...
    if (joint_state->position.size() < DOF || joint_state->velocity.size() < DOF) {
        ROS_WARN("[updateDtToolJacobian/JointStateCallback] Expected 3 joint positions and velocities, received %zu positions and %zu velocities", 
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "updateDtToolJacobian");
    ros::NodeHandle nh;
    ...
    if (!my_shared_lib.initializeSharedLib()) {
        ROS_ERROR("[updateDtToolJacobian] Failed to initialize shared library.");
        return -1;
    }
