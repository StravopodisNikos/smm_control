...
// Sequence for waypoint indices: 1->2->3->2
std::vector<int> sequence = {0, 1, 2, 1};
...
void publishDesiredState() {
    ...
    // Get the current waypoint and velocity from the sequence
    int current_index = sequence[current_sequence_index];
    ...
    // Increment the sequence index, and loop back to the start of the sequence
    current_sequence_index = (current_sequence_index + 1) % sequence.size();
}

int main(int argc, char** argv) {
    ros::init(argc, argv, "publishCartesianPathLoop_idosc_simple");
    ros::NodeHandle nh;

    // Load parameters
    if (!loadParameters(nh)) {
        ROS_ERROR("[publishCartesianPathLoop_idosc_simple] Failed to load parameters.");
        return -1;
    }
    ...
    while (ros::ok()) {
        ...
        ros::spinOnce();
        loop_rate.sleep();  // Sleep for 5 seconds
    }
