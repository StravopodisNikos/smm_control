/*
 *  [25-9-24]   Node cpp file for [idosc_simple] implementation of idosc controller
 *              - subscribes to gazebo joint states and publishes joint positions and
 *                velocities to a single topic "/joint_current_state"
 */
...
// Global variables for joint positions and velocities
...
void jointStatesCallback(const sensor_msgs::JointState::ConstPtr& joint_state, 
    ...
    if (joint_state->position.size() < DOF) {
        ROS_WARN("[updateJointState_idosc_simple/jointStatesCallback] Expected 3 joint positions, received %zu", joint_state->position.size());
        return;
    }

    // Check if joint state contains at least 3 velocities
    if (joint_state->velocity.size() < DOF) {
        ROS_WARN("[updateJointState_idosc_simple/jointStatesCallback] Expected 3 joint velocities, received %zu", joint_state->velocity.size());
        return;
    }
    ...
    ROS_INFO("[updateJointState_idosc_simple/jointStatesCallback] Updating joint states with positions and velocities");
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "updateJointState_idosc_simple");
    ros::NodeHandle nh;

    // Debug statement
    ROS_INFO("[updateJointState_idosc_simple] Initializing robot structure");

    // Initialize the robot structure
    RobotAbstractBase* robot_ptr = new Structure3Pseudos();

    // Debug statement
    ROS_INFO("[updateJointState_idosc_simple] Initializing shared library");
    ...
    if (!my_shared_lib.initializeSharedLib()) {
        ROS_ERROR("[updateJointState_idosc_simple] Failed to initialize shared library.");
        return -1;
    }

    // Initialize the shared library for robot analytical solvers using screws
    ROS_INFO("[updateJointState_idosc_simple] Initialized Shared Library.");
    ...
    // Publisher for idosc current joint state
