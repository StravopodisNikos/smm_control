...
void errorStateCallback(const smm_control::FasmcError::ConstPtr& msg) {
    ...
    ROS_INFO("[updateParams_hridosc_simple/errorStateCallback] Position Error: x=%.3f, y=%.3f, z=%.3f", 
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "updateParams_hridosc_simple");
    ros::NodeHandle nh;

    // Load fixed parameters from YAML file
    if (!loadParameters(nh)) {
        ROS_ERROR("[updateParams_hridosc_simple] Failed to load parameters. Exiting.");
        return -1;
    }

    // Subscribe to topics
    ros::Subscriber error_state_sub = nh.subscribe("/fasmc_error_state", 10, errorStateCallback); // temporary we use the fasmc error state, since it is in C-space

    // Publisher for calculated eta and epsilon values
    adaptive_params_pub = nh.advertise<smm_control::FasmcAdaptiveParams>("/hridosc_adaptive_params", 10); 
