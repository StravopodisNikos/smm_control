...
void publishCombinedTorque(ros::Publisher& torque_pub) {
    if (received_robust_term && received_dyn_term) {
        ...
        ROS_INFO("[updateTorqueCommand_fasmc_simple] Final Torques: [%f, %f, %f]",
                 combined_torque_msg.torques[0],
                 combined_torque_msg.torques[1],
                 combined_torque_msg.torques[2]);
    } else {
        ROS_WARN("[updateTorqueCommand_fasmc_simple] Waiting for both /fasmc_robust_term and /fasmc_dyn_term messages.");
    }
}

int main(int argc, char** argv) {
    ros::init(argc, argv, "updateTorqueCommand_fasmc_simple");
    ros::NodeHandle nh;

    // Subscribers for the torque terms
    ros::Subscriber robust_term_sub = nh.subscribe("/fasmc_robust_term", 10, robustTermCallback);
    ros::Subscriber dyn_term_sub = nh.subscribe("/fasmc_dyn_term", 10, dynTermCallback);
