...
void setupFuzzyEngine() {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        alpha[i]->addTerm(new fl::Gaussian("AO1", 0.001, 0.0005)); 
        alpha[i]->addTerm(new fl::Gaussian("AO2", 0.01, 0.005));
        alpha[i]->addTerm(new fl::Gaussian("AO3", 0.1, 0.05));
        ...
        beta[i]->addTerm(new fl::Gaussian("BO1", 0.001, 0.0005)); 
        beta[i]->addTerm(new fl::Gaussian("BO2", 0.01, 0.005));
        beta[i]->addTerm(new fl::Gaussian("BO3", 0.1, 0.05));
        ...
        gammaParams[i]->setRange(0.01, 10.0);
        gammaParams[i]->addTerm(new fl::Gaussian("GO1", 0.1, 0.05)); 
        gammaParams[i]->addTerm(new fl::Gaussian("GO2", 0.25, 0.15));
        gammaParams[i]->addTerm(new fl::Gaussian("GO3", 0.50, 0.25));
...
void errorStateCallback(const smm_control::FasmcError::ConstPtr& msg) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        fuzzy_params_msg.alpha[i] = alpha[i]->getValue();         // updated to getValue()
        fuzzy_params_msg.beta[i] = beta[i]->getValue();           // updated to getValue()
        fuzzy_params_msg.gamma[i] = gammaParams[i]->getValue();   // renamed to gammaParams
    }
