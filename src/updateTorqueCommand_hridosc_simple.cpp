...
void publishCombinedTorque(ros::Publisher& torque_pub) {
    if (received_robust_term && received_dyn_term) {
        ...
        ROS_INFO("[updateTorqueCommand_hridosc_simple/publishCombinedTorque] Final Torques: [%f, %f, %f]",
                 combined_torque_msg.torques[0],
                 combined_torque_msg.torques[1],
                 combined_torque_msg.torques[2]);
    } else {
        ROS_WARN("[updateTorqueCommand_hridosc_simple/publishCombinedTorque] Waiting for both /ridosc_robust_term and /ridosc_dyn_term messages.");
    }
}

int main(int argc, char** argv) {
    ros::init(argc, argv, "updateTorqueCommand_hridosc_simple");
    ros::NodeHandle nh;

    // Subscribers for the torque terms
    ros::Subscriber robust_term_sub = nh.subscribe("/hridosc_robust_term", 10, robustTermCallback);
