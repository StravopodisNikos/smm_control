...
bool getDynamicsFromService(ros::NodeHandle& nh, bool get_JacobianMatrix, bool get_LambdaMatrix, bool get_GammaVector, bool get_FgVector) {
    ...
    if (client.call(srv)) {
        ...
        ROS_ERROR("[serverUncertaintyMatrices/getDynamicsFromService] Failed to call service.");
        return false;
    }
}

// Service callback
bool handleGetUncertaintyDynamics(smm_control::GetUncertaintyDynamics::Request &req,
                                   smm_control::GetUncertaintyDynamics::Response &res,
                                   ros::NodeHandle& nh) {
    ...
    if (!getDynamicsFromService(nh, false, req.get_Lambda_un,  req.get_Gamma_un, req.get_Fg_un)) {
        ROS_ERROR("[serverUncertaintyMatrices] Failed to get dynamics from service.");
        return false;
    }
...
bool loadParameters(ros::NodeHandle& nh, float& lambda_un, float& gamma_un, float& fgrav_un) {
    ...
    ROS_INFO("[serverUncertaintyMatrices/loadParameters] Loaded parameters: lambda_un=%f, gamma_un=%f, fgrav_un=%f", 
             lambda_un, gamma_un, fgrav_un);
    return true;
}

int main(int argc, char **argv) {
    ros::init(argc, argv, "serverUncertaintyMatrices");
    ros::NodeHandle nh;

    if (!loadParameters(nh, lambda_scale, gamma_scale, gravity_scale)) {
        ROS_ERROR("[serverUncertaintyMatrices] Failed to load parameters. Exiting.");
        return -1;
    }
    ...
    // Advertise the GetUncertaintyDynamics service
    ros::ServiceServer service = nh.advertiseService<smm_control::GetUncertaintyDynamics::Request,
                                                     smm_control::GetUncertaintyDynamics::Response>(
        "GetUncertaintyDynamics",
